...
import priv.scor.enums.PeriodType;
...
public class DatetimeUtilies {
    ...
    public static Date getDate(String time, PeriodType periodType, Integer periodValue) {
        Calendar c = Calendar.getInstance();
        Integer year = Integer.parseInt(time);
        if(PeriodType.By_year.equals(periodType))
        {
            //按年
            c.set(year, 0, 1);
        }
        else if(PeriodType.By_quarter.equals(periodType))
        {
            //按季度
            int month = (periodValue-1)*3;
            c.set(year, month, 1);
        }
        else
        {
            //按月份
            c.set(year, periodValue-1, 1);
        }
